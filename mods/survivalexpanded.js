...
// Auto Seller: a placed machine that sells any pixel that touches it
elements.auto_seller = {
    color: "#22bb44",
    colorObject: {"r": 34, "g": 187, "b": 68},
    name: "auto_seller",
    desc: "Sells any element that touches it for its market value in Gold Coins.",
    category: "tools",
    tick: function(pixel) {
        for (var i = 0; i < adjacentCoords.length; i++) {
            ...
            if (!isEmpty(x, y, true)) {
                ...
                if (!neighbor || neighbor.element === "auto_seller") continue;
                var worth = elementWorth[neighbor.element];
                if (worth === undefined || worth === 0 || worth < 0) continue;
                deletePixel(x, y);
                survivalAdd("gold_coin", worth, true);
                survivalUpdate("gold_coin");
                survivalSave();
            }
        }
    }
...
// Buy 5 random blocks from the shop for a fixed price
var RANDOM_PACK_PRICE = 2000;
function survivalBuyRandom() {
    if (!settings.survival.gold_coin || settings.survival.gold_coin < RANDOM_PACK_PRICE) {
        alert("You can't afford the Random Pack! It costs $" + RANDOM_PACK_PRICE);
        return;
    }
    survivalRemove("gold_coin", RANDOM_PACK_PRICE);
    var shopKeys = Object.keys(survivalShop);
    var gained = [];
    for (var i = 0; i < 5; i++) {
        var pick = shopKeys[Math.floor(Math.random() * shopKeys.length)];
        var amount = 1;
        var elemname = pick;
        if (pick.indexOf("*") !== -1) { amount = parseInt(pick.split("*")[1]); elemname = pick.split("*")[0]; }
        survivalAdd(elemname, amount, true);
        gained.push(amount + "x " + elemname.replace(/_/g," "));
    }
    survivalSave();
    alert("Random Pack! You got:\n" + gained.join("\n"));
}

// Auto-sell: sells all pixels on canvas at regular intervals
var autoSellInterval = null;
var AUTO_SELL_MS = 3000; // sell every 3 seconds
function survivalToggleAutoSell() {
    if (autoSellInterval) {
        clearInterval(autoSellInterval);
        autoSellInterval = null;
        document.getElementById("autoSellBtn").style.backgroundColor = "#888";
        document.getElementById("autoSellBtn").innerText = "AutoSell: OFF";
    } else {
        autoSellInterval = setInterval(function() {
            if (!currentPixels) return;
            for (var i = currentPixels.length - 1; i >= 0; i--) {
                var pixel = currentPixels[i];
                if (!pixel || !pixel.element) continue;
                var worth = elementWorth[pixel.element];
                if (worth === undefined || worth === 0 || worth < 0) continue;
                deletePixel(pixel.x, pixel.y);
                survivalAdd("gold_coin", worth, true);
            }
        ...
        document.getElementById("autoSellBtn").style.backgroundColor = "#22bb44";
        document.getElementById("autoSellBtn").innerText = "AutoSell: ON";
    }
}
...
window.addEventListener("load",function(){
    ...
    shopDiv.insertAdjacentHTML("beforeend",`<p><button style="background-color:#aa44ff;color:#fff" class="elementButton" title="Buy 5 random blocks from the shop for $${RANDOM_PACK_PRICE}" onclick="survivalBuyRandom()">🎲 Random Pack<span style='font-family:Arial;font-size:1.15em'> ($${RANDOM_PACK_PRICE})</span></button></p>`);
    shopDiv.insertAdjacentHTML("beforeend",`<p><button id="autoSellBtn" style="background-color:#888;color:#fff" class="elementButton" title="Automatically sells all pixels on the canvas every 3 seconds" onclick="survivalToggleAutoSell()">AutoSell: OFF</button></p>`);
